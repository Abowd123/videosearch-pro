...
// يخبر fluent-ffmpeg بمسار برنامج ffmpeg الثنائي (يأتي جاهزًا مع حزمة ffmpeg-static)
...
app.get('/api/download', async (req, res) => {
  ...
  try {
    ...
    if (format === 'mp3') {
      // تحويل حقيقي إلى mp3: نجلب أفضل مسار صوتي متاح من يوتيوب (m4a/webm)
      // ثم نمرّره عبر ffmpeg لتحويله إلى mp3 أثناء البث (بدون حفظ ملف مؤقت)
      const ytStream = await youtube.download(videoId, {
        type: 'audio',
        quality: 'best',
        format: 'mp4'
      });

      // youtubei.js يُعيد ReadableStream (واجهة الويب)، ونحوّله إلى Node stream
      // عادي حتى يقبله fluent-ffmpeg كمدخل
      const nodeInputStream = PassThrough.fromWeb
        ? PassThrough.fromWeb(ytStream)
        : ytStream; // احتياط في حال كانت النسخة تُعيد Node stream مباشرة

      res.setHeader('Content-Disposition', `attachment; filename="${safeTitle}.mp3"`);
      res.setHeader('Content-Type', 'audio/mpeg');

      ffmpeg(nodeInputStream)
        ...
        .on('error', (err) => {
          ...
          if (!res.headersSent) {
            res.status(500).json({ error: { message: 'فشل التحويل إلى mp3: ' + err.message } });
          } else {
            res.end();
          }
        })
